...
window.Mogo.search = function () {
    ...
    function send_ajax(search_data) {
        ...
        /*  axios({
             method: 'get',
             url: '/api/items/filter/search/' + search_data,
         }).then(response => {
            ...
            if (response.status == 1) {
                if (response.data.items.length > 0) {
                    ...
                    for (var i in response.data.items) {
                        if (response.data.items[i].category.active == 1) {
                            current_results.push({
                                url: '/items/' + response.data.items[i].sku,
                                thumbnail: response.data.items[i].files.length > 0 ? '/storage/' + response.data.items[i].files[0].thumb_path : '/images/item-placeholder.png',
                                thumbnail2x: response.data.items[i].files.length > 0 ? '/storage/' + response.data.items[i].files[0].path : '/images/item-placeholder.png',
                                title: response.data.items[i].name,
                                price: response.data.items[i].price
                            })
                        }
                    }
                    data = {
                        results_count: current_results.length,
    ...
    function searchResultsToHtml(resultsInJson) {
        ...
        if (resultsInJson.results_count < 1) {
            ...
            $(resultsInJson.results).each(function (key, value) {
                ...
                html += '<span style="float: right; font-size: 18px"><b>$' + value.price + '</b></span>';
                html += '</div>';
            });
        }
...
window.Mogo.revolutionSlider = {
    resizeRev: function (options, new_rev_obj, bp_arr) {
        ...
        if($("script[src*='/vendor/revolution/js']").length >0){
            var src = ($("script[src*='/vendor/revolution/js']").first().attr('src')).split('/vendor/revolution/js/');
            rev_obj.jsFileLocation = src[0] + rev_obj.jsFileLocation;
