...
class OrderQuoteEmail extends Mailable
{
    ...
    {
        $subject = 'Properos Quote';
        if(isset($this->order_data->subject)){
            $subject = $this->order_data->subject;
        }
        return $this->from($this->order_data->from)->subject($subject)->view('themes.'.Theme::get().'.emails.order-quote');
